      ...
      >
        ...
        <AnimatePresence>
          {stage === "idle" && (
            <motion.div
              className="absolute"
              style={{ left: "40%", top: "60%", transform: "translate(-50%, -50%)", zIndex: 20, width: 72, height: 72 }}
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0, opacity: 0, transition: { duration: 0.2 } }}
              transition={{ delay: 0.6, duration: 0.7, type: "spring", stiffness: 160, damping: 12 }}
            >
              <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: `radial-gradient(circle at 36% 30%, #7DD3FC, ${theme.color.skyBlue} 55%, ${theme.color.skyBlueDark} 100%)`, boxShadow: ["0 6px 20px rgba(0,0,0,0.65)", "0 1px 0 rgba(255,255,255,0.2) inset", "0 -3px 8px rgba(0,0,0,0.4) inset"].join(", ") }} />
