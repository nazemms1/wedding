...
function CountdownBox({ value, label, delay, inView, theme }: {
  value: number; label: string; delay: number; inView: boolean
  theme: { card: { border: string }; font: { display: string; body: string }; color: { gold: string }; ornament: { solid: string; skyBlue?: string } }
}) {
  return (
    <motion.div className="flex flex-col items-center gap-4"
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(145deg, rgba(56,189,248,0.08) 0%, rgba(255,255,255,0.03) 100%)', border: `1px solid ${theme.card.border}`, boxShadow: `0 0 30px rgba(56,189,248,0.12), 0 8px 32px rgba(0,0,0,0.3)` }}
        animate={{ boxShadow: ['0 0 20px rgba(56,189,248,0.1)', '0 0 40px rgba(56,189,248,0.25)', '0 0 20px rgba(56,189,248,0.1)'] }}
        transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}>
        <span className="text-4xl sm:text-5xl md:text-6xl font-light tabular-nums"
          style={{ fontFamily: theme.font.display, color: '#38BDF8', textShadow: '0 0 20px rgba(56,189,248,0.5)' }}>
          {String(value).padStart(2, '0')}
        </span>
        <div className="absolute top-0 left-0 w-3.5 h-3.5 border-t-2 border-l-2" style={{ borderColor: '#38BDF8' }} />
        <div className="absolute top-0 right-0 w-3.5 h-3.5 border-t-2 border-r-2" style={{ borderColor: '#38BDF8' }} />
        <div className="absolute bottom-0 left-0 w-3.5 h-3.5 border-b-2 border-l-2" style={{ borderColor: '#38BDF8' }} />
        <div className="absolute bottom-0 right-0 w-3.5 h-3.5 border-b-2 border-r-2" style={{ borderColor: '#38BDF8' }} />
      </motion.div>
      <span className="text-[11px] tracking-[0.35em] uppercase" style={{ color: '#38BDF8', fontFamily: theme.font.body, opacity: 0.75 }}>
        {label}
      </span>
    </motion.div>
  )
...
export function BisherDateSection() {
  ...
  return (
    <motion.section ref={ref as React.RefObject<HTMLElement>}
      ...
      initial={{ opacity: 0, y: 50 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9 }}>
      ...
      <motion.div className="absolute pointer-events-none" style={{ inset: 0, background: theme.bg.glow }}
        animate={{ opacity: [0.5, 1, 0.5], scale: [1, 1.1, 1] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }} />

      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        {Array.from({ length: 24 }).map((_, i) => (
          <motion.div key={i} className="absolute rounded-full"
            style={{ width: 1.2 + (i % 5) * 0.6, height: 1.2 + (i % 5) * 0.6, left: `${(i * 3.9 + 5) % 100}%`, top: `${(i * 5.8 + 1) % 100}%`, background: (theme.dust as unknown as string[])[i % 4] }}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none" aria-hidden="true">
        <motion.div className="w-[700px] h-[700px] rounded-full" style={{ border: '1px solid rgba(56,189,248,0.06)' }}
          animate={{ rotate: 360 }} transition={{ duration: 40, repeat: Infinity, ease: 'linear' }} />
        <motion.div className="absolute w-[500px] h-[500px] rounded-full" style={{ border: '1px solid rgba(56,189,248,0.04)' }}
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        <motion.h3 className="font-normal mb-3"
        ...
        <motion.p className="text-base tracking-widest mb-14"
        ...
        {isPast ? (
          <div className="flex flex-col items-center gap-6">
            <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.3, duration: 0.9, type: 'spring', stiffness: 100 }}>
              <svg width="72" height="48" viewBox="0 0 72 48" fill="none" aria-hidden="true">
                ...
                <circle cx="48" cy="24" r="18" stroke={theme.color.skyBlue} strokeWidth="2.5" fill="none" opacity="0.9" />
                <motion.circle cx="24" cy="24" r="18" stroke={theme.color.gold} strokeWidth="1" fill="none"
                  animate={{ r: [18, 24, 18], opacity: [0.4, 0, 0.4] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeOut' }} />
                <motion.circle cx="48" cy="24" r="18" stroke={theme.color.skyBlue} strokeWidth="1" fill="none"
                  animate={{ r: [18, 24, 18], opacity: [0.4, 0, 0.4] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeOut', delay: 0.4 }} />
              </svg>
            </motion.div>
            <motion.p className="text-xs tracking-[0.45em] uppercase" style={{ color: theme.color.skyBlue, fontFamily: theme.font.body }}
              initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.45, duration: 0.7 }}>
              {t.weAreMarried}
            </motion.p>
            <motion.h3 className="font-normal text-center"
              style={{ fontFamily: theme.font.display, fontSize: 'clamp(2rem, 6vw, 4rem)', color: theme.color.goldLight, textShadow: theme.shadow.name, lineHeight: 1.2 }}
              initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.55, duration: 0.9, ease: [0.22, 1, 0.36, 1] }}>
              {t.congratsHeading}
            </motion.h3>
            <motion.div className="flex items-center gap-3" initial={{ scaleX: 0 }} animate={inView ? { scaleX: 1 } : {}} transition={{ delay: 0.75, duration: 0.8 }}>
              <div className="h-px w-16 sm:w-24" style={{ background: `linear-gradient(to right, transparent, ${theme.ornament.mid})` }} />
              <motion.div className="w-2 h-2 rotate-45" style={{ background: theme.color.skyBlue }}
            ...
            <motion.p className="text-base md:text-lg leading-relaxed text-center max-w-lg"
              style={{ color: theme.color.textOnDark, fontWeight: 300, fontFamily: theme.font.body }}
              initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.85, duration: 0.8 }}>
              {t.congratsSubtitle}
            </motion.p>
            <motion.p className="text-sm italic" style={{ color: theme.color.tagline, fontFamily: theme.font.display }}
              initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 1.05, duration: 0.8 }}>
              {t.congratsTagline}
            </motion.p>
          </div>
        ) : (
          <div className="flex items-start justify-center gap-3 sm:gap-5 md:gap-7 flex-wrap">
            {boxes.map((box, i) => (
              <div key={box.label} className="flex items-start gap-3 sm:gap-5 md:gap-7">
                <CountdownBox value={box.value} label={box.label} delay={0.4 + i * 0.12} inView={inView} theme={theme} />
                {i < boxes.length - 1 && (
                  <motion.span className="text-3xl mt-7 sm:mt-9" style={{ color: theme.color.skyBlue, opacity: 0.5 }}
                    initial={{ opacity: 0 }} animate={inView ? { opacity: 0.5 } : {}} transition={{ delay: 0.5 + i * 0.12 }}>
                    :
                  </motion.span>
                )}
              </div>
        ...
        <motion.div className="flex items-center justify-center gap-3 mt-14"
