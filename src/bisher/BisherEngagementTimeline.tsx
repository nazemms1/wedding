...
function EventCard({ event, align, theme }: {
  event: { date: string; title: string; description: string }
  align: 'start' | 'end'
  theme: { color: { gold: string; goldLight: string; skyBlue: string; textOnDark: string }; font: { body: string; display: string } }
}) {
  return (
    <div className="py-8 px-1" style={{ textAlign: align === 'end' ? 'right' : 'left' }}>
      <p className="text-xs tracking-[0.3em] uppercase mb-2" style={{ color: theme.color.skyBlue, fontFamily: theme.font.body, opacity: 0.75 }}>
        {event.date}
      </p>
      <h4 className="font-normal mb-2" style={{ fontFamily: theme.font.display, fontSize: 'clamp(1.1rem, 2.5vw, 1.5rem)', color: theme.color.goldLight }}>
        {event.title}
      </h4>
      <p className="text-sm leading-relaxed" style={{ color: theme.color.textOnDark, fontWeight: 300, fontFamily: theme.font.body, opacity: 0.8 }}>
        {event.description}
      </p>
    </div>
  )
...
export function BisherEngagementTimeline() {
  const { t, theme, config, dir } = useBisher()
  ...
  return (
    <motion.section ref={ref as React.RefObject<HTMLElement>}
      ...
      <div aria-hidden="true" className="absolute inset-0"
        style={{ backgroundImage: `url(${config.sectionImages.engagment})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }} />
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        {Array.from({ length: 24 }).map((_, i) => (
          <motion.div key={i} className="absolute rounded-full"
            style={{ width: 1.2 + (i % 5) * 0.6, height: 1.2 + (i % 5) * 0.6, left: `${(i * 3.7 + 4) % 100}%`, top: `${(i * 5.5 + 2) % 100}%`, background: (theme.dust as unknown as string[])[i % 4] }}
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        <div className="text-center mb-20">
          ...
          <motion.p className="text-sm leading-relaxed" style={{ color: theme.color.textMuted, fontWeight: 300, fontFamily: theme.font.body }}
        ...
        <div className="relative">
          <motion.div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px"
            style={{ background: `linear-gradient(to bottom, transparent, ${theme.ornament.skyHalf} 10%, ${theme.ornament.skyHalf} 90%, transparent)` }}
            initial={{ scaleY: 0, originY: 0 }} animate={inView ? { scaleY: 1 } : {}} transition={{ delay: 0.5, duration: 1.2, ease: 'easeOut' }} />
              ...
              return (
                <motion.div key={i} className="relative flex items-center"
                  ...
                  <div className={`w-1/2 ${isLeft ? 'pr-10 text-end' : 'pl-10'}`}>
                    {isLeft && <EventCard event={event} align="end" theme={theme} />}
                  </div>
                  <div className="absolute left-1/2 -translate-x-1/2 z-10 flex flex-col items-center">
                    <motion.div className="w-10 h-10 rounded-full flex items-center justify-center text-base"
                      style={{
                        ...
                        color: '#071628', fontFamily: theme.font.display,
                      }}
